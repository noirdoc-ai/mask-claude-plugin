...
- Self-hosted marketplace manifest (`.claude-plugin/marketplace.json`) and plugin
  manifest (`noirdoc/.claude-plugin/plugin.json`), plus starter workspace config
  template (`noirdoc/templates/config.toml`).
...
- 48 unit tests covering glob matching, config parsing, tool-input extraction,
  allowlist precedence, and end-to-end script integration via subprocess.
